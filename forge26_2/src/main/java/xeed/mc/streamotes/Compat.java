...
public class Compat {
	...
	public static void sendToastMessage(Component title, Component text) {
		...
		mc.execute(() -> mc.gui.toastManager().addToast(new SystemToast(Compat.TOAST_TYPE, title, text)));
	}
	...
	public static class Texture implements AutoCloseable {
		...
		public void upload(String label, NativeImage buffer) {
			...
			if (texture == null) {
				texture = dev.createTexture(label, GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING, GpuFormat.RGBA8_UNORM, buffer.getWidth(), buffer.getHeight(), 1, 1);
			}
			if (view == null) {
		...
		public class CustomSetup extends RenderSetup {
			public CustomSetup() {
				super(RenderPipelines.TEXT, Collections.emptyMap(), true, false, LayeringTransform.NO_LAYERING,
					OutputTarget.MAIN_TARGET, TextureTransform.DEFAULT_TEXTURING, OutlineProperty.NONE, false, false);
			}

			@Override
			@NonNull
			public List<PreparedRenderType.Texture> prepareTextures(final @NonNull TextureManager textureManager, final SamplerCache samplerCache, final @NonNull GpuTextureView overlayTexture, final @NonNull GpuTextureView lightmapTexture) {
				return List.of(new PreparedRenderType.Texture(texture.getLabel(), view, samplerCache.getSampler(AddressMode.CLAMP_TO_EDGE, AddressMode.CLAMP_TO_EDGE, FilterMode.LINEAR, FilterMode.LINEAR, false)));
			}
		}
	}
}
