...
public class CompatServer {
	...
	public static void onInitializeServer(RegisterPayloadHandlersEvent event) {
		event.registrar(JsonPayload.PACKET_ID.id().getNamespace()).optional().playToClient(JsonPayload.PACKET_ID, JsonPayload.PACKET_CODEC, (packet, context) -> Streamotes.INSTANCE.onReceiveJsonPacket(packet.json()));
	}
